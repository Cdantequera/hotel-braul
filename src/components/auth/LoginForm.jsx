...
// URL SEGURA: Si no encuentra la variable, usa directamente Render
...
function LoginForm({ onLogin, onError }) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (response.data.ok) onLogin(response.data);

    } catch (error) {
      console.error("Error login:", error);
      if (onError) onError(error.response?.data?.message || "Error al conectar con el servidor");
    }
  };
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="space-y-1">
        ...
        <div className="relative">
            ...
            <input type="email" placeholder="tu_email@hotel.com" className={`${inputClasses} ${errors.email ? "border-red-500" : ""}`} {...register("email")} />
        </div>
      ...
      <div className="space-y-1">
        ...
        <div className="relative">
            ...
            <input type="password" placeholder="Tu Password *****" className={`${inputClasses} ${errors.password ? "border-red-500" : ""}`} {...register("password")} />
        </div>
      ...
      <button type="submit" disabled={isSubmitting} className="group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-bold rounded-md text-white bg-linear-to-r from-[#C5A572] to-[#b08d4e] hover:from-[#b08d4e] hover:to-[#C5A572] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#C5A572] transition-all duration-300 shadow-lg uppercase tracking-widest disabled:opacity-50">
        {isSubmitting ? <><Loader2 className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" /> Iniciando...</> : "Iniciar Sesión"}
      </button>
    </form>
  );
}
