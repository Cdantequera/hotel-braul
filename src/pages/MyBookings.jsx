...
const MyBookings = () => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "confirmed":
        return (
          <span className="flex items-center text-green-500 bg-green-500/10 px-3 py-1 rounded-full text-xs uppercase tracking-wider font-bold">
            <CheckCircle className="w-4 h-4 mr-1" /> Confirmada
          </span>
        );
      case "pending":
        return (
          <span className="flex items-center text-yellow-500 bg-yellow-500/10 px-3 py-1 rounded-full text-xs uppercase tracking-wider font-bold">
            <Clock className="w-4 h-4 mr-1" /> Pendiente
          </span>
        );
      case "cancelled":
        return (
          <span className="flex items-center text-red-500 bg-red-500/10 px-3 py-1 rounded-full text-xs uppercase tracking-wider font-bold">
            <XCircle className="w-4 h-4 mr-1" /> Cancelada
          </span>
        );
      default:
        return null;
  ...
  return (
    <div className="bg-black min-h-screen pt-24 pb-20">
      <div className="container mx-auto px-4 max-w-5xl">
        <h1 className="text-4xl font-serif font-bold text-white mb-12 uppercase tracking-widest">
          Mis Reservas
        </h1>
        ...
        {bookings.length === 0 && !error ? (
          <div className="text-center py-20 text-gray-500">
            ...
            <Link
              to="/rooms"
              className="mt-6 inline-block px-8 py-3 bg-[#C5A572] text-black font-bold uppercase tracking-widest text-sm hover:bg-white transition-all"
            >
              Ver Habitaciones
            </Link>
          </div>
          ...
          <div className="space-y-6">
            {bookings.map((booking) => (
              <div
                key={booking._id}
                className="bg-zinc-900 border border-white/5 rounded-2xl overflow-hidden flex flex-col md:flex-row hover:border-[#C5A572]/30 transition-all"
              >
                <div className="w-full md:w-1/3 h-48 md:h-auto relative">
                  <img
                    ...
                    onError={(e) => {
                      e.target.src = "https://via.placeholder.com/400x300?text=Habitacion";
                    }}
                  />
                </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      <div className="bg-black/40 p-3 rounded border border-white/5">
                        <p className="text-[10px] text-[#C5A572] uppercase tracking-widest mb-1">
                          Entrada
                        </p>
                  ...
                  <div className="flex flex-col border-t border-white/10 pt-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex gap-3">
                        ...
                        {booking.paymentStatus === "paid" && (
                          <span className="px-6 py-2 bg-green-500/10 text-green-500 border border-green-500/20 text-xs font-bold uppercase tracking-widest rounded">
                            Pagado
                          </span>
                        )}
                      </div>
