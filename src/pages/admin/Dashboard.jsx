...
const Dashboard = () => {
  ...
  const getStatusStyle = (status) => {
    switch (status) {
      case "confirmed":
        return { label: "Confirmado", cls: "bg-green-900/30 text-green-400" };
      case "pending":
        return { label: "Pendiente", cls: "bg-yellow-900/30 text-yellow-400" };
      case "cancelled":
        return { label: "Cancelado", cls: "bg-red-900/30 text-red-400" };
      case "completed":
        return { label: "Completado", cls: "bg-blue-900/30 text-blue-400" };
      default:
        return { label: status, cls: "bg-gray-900/30 text-gray-400" };
    }
  };
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="bg-black border border-white/10 p-6 rounded-xl animate-pulse h-36"
            />
          ))}
        </div>
      ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-black border border-white/10 rounded-xl p-6">
              ...
              <div className="w-full bg-zinc-800 rounded-full h-2">
                <div
                  ...
                  style={{
                    width:
                      stats.totalRooms > 0
                        ? `${Math.round((stats.pendingBookings / stats.totalRooms) * 100)}%`
                        : "0%",
                  }}
                ></div>
              </div>
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="text-blue-400 font-bold">
                  {Math.max(0, stats.totalRooms - stats.activeBookings - stats.pendingBookings)}{" "}
                  Habs
                </span>
              </div>
              <div className="w-full bg-zinc-800 rounded-full h-2">
          ...
          <div className="mt-6 flex space-x-4">
            <button className="flex-1 bg-zinc-900 border border-gray-700 text-gray-300 py-2 rounded text-sm hover:border-[#C5A572] transition-colors">
              Solicitar Limpieza
            </button>
            <button className="flex-1 bg-zinc-900 border border-gray-700 text-gray-300 py-2 rounded text-sm hover:border-red-500 transition-colors">
              Reportar Avería
            </button>
          </div>
        </div>
            ...
            {loading ? (
              ...
              recentBookings.slice(0, 4).map((booking, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="flex items-start space-x-3 border-b border-white/5 pb-3 last:border-0 last:pb-0"
                  >
