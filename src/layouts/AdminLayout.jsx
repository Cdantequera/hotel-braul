...
const AdminLayout = () => {
  ...
  return (
    <div className="flex h-screen bg-zinc-950 text-white font-sans overflow-hidden">
      {/* --- 1. SIDEBAR --- */}
      <aside
        className={`${sidebarOpen ? "w-64" : "w-20"} bg-black border-r border-white/10 transition-all duration-300 flex flex-col z-20`}
      >
        {/* Logo Area */}
        <div className="h-16 flex items-center justify-center border-b border-white/10">
          {sidebarOpen ? (
            <h1 className="text-xl font-serif font-bold text-[#C5A572] tracking-widest">
              BRA'UL <span className="text-white text-xs">ADMIN</span>
            </h1>
          ) : (
        ...
        <header className="h-16 bg-black/50 backdrop-blur-md border-b border-white/10 flex justify-between items-center px-6">
          <div className="flex items-center">
            ...
            <div className="ml-4 hidden md:flex items-center bg-zinc-900 border border-gray-700 rounded-full px-4 py-1.5 focus-within:border-[#C5A572] transition-colors">
              ...
              <input
                type="text"
                placeholder="Buscar reserva, habitación..."
                className="bg-transparent border-none focus:ring-0 text-sm text-white ml-2 w-64 outline-none"
              />
            </div>
          </div>
